...
class Usertracking {
	...
	public function initialize() {
		...
		// echo (int)$this->CI->config->load('usertracking');exit;
		if (! $this->CI->config->load('usertracking') or $this->CI->config->item('usertracking') === FALSE) {
	...
	public function get_logged_in_user($tracking_data){

		if( preg_match('/api/', $tracking_data['request_uri']) ){
			$data_headers = $this->CI->input->request_headers(TRUE);

			$accesstoken = isset($data_headers['Access-Token']) ? trim(str_replace("bearer", "", $data_headers['Access-Token'])) : false;

			if(!$accesstoken){
				$accesstoken = isset($data_headers['Token']) ? trim(str_replace("bearer", "", $data_headers['Token'])) : false;
			}

			return $accesstoken;
		}
		
		$adminData = config_item('session_data_key');
		$admin_id = $user_id = 0;
		if(isset($this->CI->session->$adminData)) {
            $tempAdminData = $this->CI->session->$adminData;
			$admin_id = $tempAdminData['user_id'];
		}
		
		$userData = 'user_'.config_item('session_data_key');
		if(isset($this->CI->session->$userData)) {
			$tempUserData = $this->CI->session->$userData;
			$user_id = $tempUserData['user_id'];
		}
		
		$returnData = array('admin_id' => (int)$admin_id, 'user_id' => (int)$user_id);
		return $returnData;
 	}
	...
	public function track_this() {
		...
		$input_data['http_method'] = $this->CI->input->method();
		$input_data['post_parameters'] = json_encode($this->CI->input->post());
		$input_data['get_parameters'] = json_encode($this->CI->input->get());
		$input_data['raw_parameters'] = $this->CI->input->raw_input_stream;
		$input_data['insert_datetime'] = date('Y-m-d H:i:s');
			...
			if (! $this->CI->load->$class_type($class_name)) {
				...
				else // Do it!
				{
					if ($class_type == 'helper')
						...
						$input_data['user_identifier'] = call_user_func_array(array(
							$this->CI->$class_name,
							$function_name
						), $the_args);
				}
		...
		$temp = $this->get_logged_in_user($input_data);
		
		$input_data['user_identifier'] = $temp['user_id'];
		$input_data['admin_identifier'] = $temp['admin_id'];
				...
				if (! $this->CI->load->$class_type($class_name)) {
					...
					else // Do it!
					{
						if ($class_type == 'helper')
							...
							$curr_filter_result = call_user_func_array(array(
								$this->CI->$class_name,
								$function_name
							), $the_args);
					}
	...
	private function build_database_table() {
		...
		foreach ($this->needed_fields as $curr_nf) {
			...
			if (isset($curr_nf['null']))
				$new_fields[$name]['null'] = $curr_nf['null'];

			if (isset($curr_nf['default']))
				$new_fields[$name]['default'] = $curr_nf['default'];

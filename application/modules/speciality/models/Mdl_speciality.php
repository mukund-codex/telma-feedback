...
class Mdl_speciality extends MY_Model {

	private $p_key = 'speciality_id';
	private $table = 'speciality';
	private $alias = 'd';
	private $fillable = ['speciality_name'];
    private $column_list = ['Name','Date'];
    private $csv_columns = ['Name'];
	...
	function get_collection( $count = FALSE, $sfilters = [], $rfilters = [], $limit = 0, $offset = 0, ...$params ) {
		...
		if(! $count) {
			$q->order_by('d.speciality_id desc');
		}
		...
		if($type == 'save') {
			return [
                [
					...
					'rules' => 'trim|required|valid_name|max_length[150]|unique_record[add.table.speciality.speciality_name.' . $this->input->post('speciality_name') .']|xss_clean'
                ],
		...
		if($type == 'modify') {
			return [
				[
					'field' => 'speciality_name',
					'label' => 'Speciality Name',
					'rules' => 'trim|required|valid_name|max_length[150]|unique_record[edit.table.speciality.speciality_name.' . $this->input->post('speciality_name'). '.speciality_id.'. $this->input->post('speciality_id') .']|xss_clean'
                ],
...
/* 
        ...
        $response['message'] = 'Congratulations! Speciality has been added successfully.';
        $response['redirectTo'] = 'speciality/lists';

...
class Mdl_bunch extends MY_Model {

	private $p_key = 'id';
	private $table = 'bunch';
	private $alias = 'd';
	private $fillable = ['code'];
    private $column_list = ['Bunch Code', 'Created'];
    private $csv_columns = ['Bunch Code','Created'];
		...
		if($type == 'save') {
			return [
                [
					'field' => 'code',
					'label' => 'Code',
					'rules' => 'trim|required|valid_mobile|unique_record[add.table.bunch.code.' . $this->input->post('code') .']|xss_clean'
				]
			];
		}
		...
		if($type == 'modify') {
			return [
                [
					...
					'rules' => 'trim|required|valid_mobile|unique_record[edit.table.bunch.code.' . $this->input->post('code'). '.doctor_id.'. $this->input->post('id') .']|xss_clean'
                ]
			];
		}
    }
	...
	function save(){
        ...
        $response['status'] = TRUE;
        $response['message'] = 'Congratulations! your Bunch Code is saved with us!.';
        $response['redirectTo'] = 'bunch/lists';

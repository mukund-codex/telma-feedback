...
class Mdl_email_logs extends MY_Model {
	...
	function get_filters() {
        return [
            ...
            [
                'field_name'=>'el|doctor_email',
                'field_label'=> 'Doctor Email',
			],
			[],
			[],
			[]
	...
	function get_collection($count = FALSE, $sfilters = [], $rfilters = [], $limit = 0, $offset = 0, ...$params) {
    	...
    	$q = $this->db->select('
			el.id as id, dr.name, el.doctor_email, el.subject, el.content, el.is_success, el.insert_dt
    	')
		->from('email_log el')
		->join('doctor dr', 'dr.doctor_id = el.doctor_id');
		...
		if(is_array($rfilters) && count($rfilters) ) {
            ...
            foreach($rfilters as $key=> $value) {
                ...
                if($key == 'from_date' && $value) {
                    $this->db->where('DATE(el.insert_dt) >=', date('Y-m-d', strtotime($value)));
                    continue;
                }
	...
	function _format_data_to_export($data){
		...
		foreach ($data as $rows) {
			...
			$records['Doctor Email'] = $rows['doctor_email'];
			$records['Subject'] = $rows['subject'];
			$records['Status'] = ($rows['is_success'] == 0) ? 'Failed' : 'Success';
